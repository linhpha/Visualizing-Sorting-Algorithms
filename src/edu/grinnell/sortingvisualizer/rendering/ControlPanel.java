	...
	public ControlPanel(NoteIndices notes, ArrayPanel panel) {
		...
		playButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Integer[] copyIndices = Arrays.copyOf(notes.indices, notes.indices.length);
				...
				for (int i = 0; i < notes.indices.length; i++) {
					events.add(new CompareEvents<Integer>(0,i));				}
					...
					public void run() {
						if (index < events.size()) {
							...
							for (int i = 0; i < e.getAffectedIndices().size(); i++) {
								scale.playNote((int) e.getAffectedIndices().get(i), 
										e.isEmphasized());
							}

							// 3. Highlight those affected indices.
							for (int x : e.getAffectedIndices()) {
								notes.highlightNote(x);
							}				
							panel.repaint();
						} else {
							this.cancel();
